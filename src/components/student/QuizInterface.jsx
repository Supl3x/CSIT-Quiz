      ...
      >
        <motion.div
          ...
          className={`relative p-12 text-center overflow-hidden ${
            results.score >= 80 ? 'bg-gradient-to-br from-emerald-600 via-green-600 to-teal-600' :
            results.score >= 60 ? 'bg-gradient-to-br from-amber-600 via-yellow-600 to-orange-600' :
        ...
        <div className="p-8">
          ...
          >
            {[
              ...
              { icon: Clock, label: 'Time', value: `${Math.floor(results.timeSpent / 60)}:${(results.timeSpent % 60).toString().padStart(2, '0')}`, color: 'purple' },
              ...
              {currentQuestion.options.map((option, index) => (
                ...
                >
                  <div className={`absolute inset-0 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity ${
                    answers[currentQuestion.id] === index ? 'opacity-100' : ''
                  }`} />
                  <div className="relative flex items-center gap-4">
                    <motion.div
                      animate={{
                        scale: answers[currentQuestion.id] === index ? [1, 1.2, 1] : 1,
                        rotate: answers[currentQuestion.id] === index ? [0, 360] : 0
                      }}
                    ...
                    <span className={`text-base font-medium ${
                      answers[currentQuestion.id] === index ? 'text-white' : 'text-slate-300'
                    }`}>{option}</span>
                  </div>
                </motion.button>
              ))}
