...
export default function AnalyticsDashboard() {
  ...
  const averageScore = attempts.length > 0 
    ? Math.round(attempts.reduce((sum, attempt) => sum + attempt.totalScore, 0) / attempts.length)
    : 0;

  const highestScore = attempts.length > 0 
    ? Math.max(...attempts.map(attempt => attempt.totalScore))
    : 0;

  const lowestScore = attempts.length > 0 
    ? Math.min(...attempts.map(attempt => attempt.totalScore))
    : 0;
