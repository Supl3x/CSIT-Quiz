...
function SortableQuestionItem({ question, onEdit, onDelete, onTogglePreview, showPreview }) {
  ...
  const category = (question.tags && question.tags.length > 0) ? question.tags[0] : 'General';
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={() => onTogglePreview(question._id)}
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            <motion.span
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium backdrop-blur-xl ${
                category === 'Programming' ? 'bg-blue-500/20 text-blue-300 border border-blue-500/30' :
                category === 'DBMS' ? 'bg-green-500/20 text-green-300 border border-green-500/30' :
                category === 'Networks' ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30' :
                category === 'AI' ? 'bg-orange-500/20 text-orange-300 border border-orange-500/30' :
          ...
          <AnimatePresence>
            {showPreview && (
              ...
              >
                {question.choices && question.choices.map((option, index) => (
                  <motion.div
      ...
      >
        ...
        <div className="p-6">
          {filteredQuestions.length === 0 ? (
            ...
            >
              <SortableContext
                items={filteredQuestions.map(q => q._id)}
                ...
                <div className="space-y-4">
                  {filteredQuestions.map((question) => (
                    <SortableQuestionItem
                      ...
                      showPreview={previewStates[question._id]}
                    />
                  ))}
                </div>
              </SortableContext>
